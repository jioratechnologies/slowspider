...
class ApiClient {
  ...
  static Future<bool> testConnection(String url) async {
    try {
      ...
      final res = await http
          .get(Uri.parse('$u/v1/auth/session'))
  ...
  Future<T> _request<T>(
    ...
    if (auth) {
      ...
      if (session.workspaceId != null) {
        headers['x-workspace-id'] = session.workspaceId.toString();
      }
    }
    ...
    final isUnauthorized =
        res.statusCode == 401 ||
        err?.contains('expired') == true ||
        err?.contains('invalid token') == true;
  ...
  Future<void> signupOtp(String email) => _request<void>(
    '/v1/auth/signup/otp',
    method: 'POST',
    body: {'email': email},
    auth: false,
  ...
  Future<void> signupVerify(String email, String code) => _request<void>(
  ...
  Future<Map<String, dynamic>> signupComplete(String email, String password) =>
      _request<Map<String, dynamic>>(
  ...
  Future<void> resetOtp(String email) => _request<void>(
    '/v1/auth/reset/otp',
    method: 'POST',
    body: {'email': email},
    auth: false,
  ...
  Future<void> resetVerify(String email, String code) => _request<void>(
  ...
  Future<Map<String, dynamic>> resetComplete(String email, String password) =>
      _request<Map<String, dynamic>>(
  ...
  Future<void> saveSortMode(SortMode mode) => _request<void>(
    '/v1/settings/sort-mode',
    method: 'PATCH',
    body: {'sortMode': mode.name},
  );
  ...
  Future<void> updateTask(int id, Map<String, dynamic> patch) =>
      _request<void>('/v1/tasks/$id', method: 'PATCH', body: patch);

  Future<void> deleteTaskForever(int id) =>
      _request<void>('/v1/tasks/$id', method: 'DELETE');
  ...
  Future<void> updateMilestone(
    ...
    '/v1/tasks/$taskId/milestones/$id',
    method: 'PATCH',
    body: patch,
  );

  Future<void> deleteMilestone(int taskId, int id) =>
      _request<void>('/v1/tasks/$taskId/milestones/$id', method: 'DELETE');
  ...
  Future<void> updateCluster(int id, Map<String, dynamic> patch) =>
      _request<void>('/v1/clusters/$id', method: 'PATCH', body: patch);

  Future<void> deleteClusterForever(int id) =>
      _request<void>('/v1/clusters/$id', method: 'DELETE');
  ...
  Future<void> updateCategory(int id, Map<String, dynamic> patch) =>
      _request<void>('/v1/categories/$id', method: 'PATCH', body: patch);

  Future<void> deleteCategory(int id) =>
      _request<void>('/v1/categories/$id', method: 'DELETE');
  ...
  Future<void> updateNote(int id, Map<String, dynamic> patch) =>
      _request<void>('/v1/notes/$id', method: 'PATCH', body: patch);

  Future<void> deleteNote(int id) =>
      _request<void>('/v1/notes/$id', method: 'DELETE');

  Future<Map<String, dynamic>> uploadUrl(String filename, int sizeBytes) =>
      _request<Map<String, dynamic>>(
        '/v1/notes/media',
        method: 'POST',
  ...
  Future<String> mediaUrl(String path) async {
    final data = await _request<Map<String, dynamic>>(
      '/v1/notes/media?path=${Uri.encodeComponent(path)}',
    );
    return data['url'] as String;
  }

  Future<Map<String, dynamic>> quota() =>
      _request<Map<String, dynamic>>('/v1/notes/media');
  ...
  Future<void> renameWorkspace(int id, String name) => _request<void>(
    '/v1/workspace/$id',
    method: 'PATCH',
    body: {'name': name},
  );
  ...
  Future<void> inviteMember(String email) => _request<void>(
    '/v1/workspace/invite',
    method: 'POST',
    body: {'email': email},
  );
  ...
  Future<void> revokeInvite(int id) =>
      _request<void>('/v1/workspace/invite/$id', method: 'DELETE');

  Future<Map<String, dynamic>> listMembers() =>
      _request<Map<String, dynamic>>('/v1/workspace/members');

  Future<void> removeMember(String userId) =>
      _request<void>('/v1/workspace/members/$userId', method: 'DELETE');

  Future<List<PendingInviteForUser>> myInvites() async {
    final data = await _request<Map<String, dynamic>>(
      '/v1/workspace/my-invites',
    );
    return ((data['invites'] as List?) ?? [])
        .map((i) => PendingInviteForUser.fromJson(i as Map<String, dynamic>))
        .toList();
  }

  Future<void> declineMyInvite(int id) =>
      _request<void>('/v1/workspace/my-invites/$id/decline', method: 'POST');
}

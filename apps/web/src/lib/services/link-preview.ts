...
// The server fetches a URL the user typed, so this is a classic SSRF sink: without a guard
// a note link of http://169.254.169.254/… or http://localhost:5432 would make our server
// probe its own network and hand the response back. Every hostname is therefore resolved
// first and rejected unless it lands on a public address.
function isPrivateAddress(ip: string): boolean {
  if (isIP(ip) === 6) {
    const v6 = ip.toLowerCase();
    if (v6 === "::1" || v6 === "::" || v6.startsWith("fc") || v6.startsWith("fd") || v6.startsWith("fe80")) return true;
    // IPv4-mapped (::ffff:a.b.c.d) tunnels the whole IPv4 problem through IPv6.
...
async function assertPublicUrl(raw: string): Promise<URL> {
  ...
  if (addresses.some((a) => isPrivateAddress(a.address))) throw new Error("That link points somewhere private.");
  return url;
}
...
export async function fetchLinkPreview(raw: string): Promise<LinkPreview> {
  ...
  const title = metaTag(html, "og:title", "twitter:title") || html.match(/<title[^>]*>([^<]*)<\/title>/i)?.[1] || url.hostname;
  const description = metaTag(html, "og:description", "twitter:description", "description") || "";
  const image = metaTag(html, "og:image", "twitter:image");

  return {
    url: url.toString(),
    title: decodeEntities(title).slice(0, 200),
    description: decodeEntities(description).slice(0, 400),
